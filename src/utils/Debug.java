 ...
 * By default, debug is ON (use setDebug)
 * By default, debug window is not visible (use setWindowView)
 * By default, debug is not logged on file  (use activateDebugLog)
 * By default, the prefix to debug messages is [DEBUG] (use setPrefix)
...
public class Debug {
	...
	static private boolean debugInWindow = false;
	...
	static public void showWindow( boolean visible ) {
		debugWindow.setVisible( visible && debugActivated );
	}
	/** Closes debug window and other resources (prior to closing application)
	 * Post: Don't make more use of Debug class on this session after this call
	 */
