...
//		System.exit(0);
	...
	private static void verbosTotales() {
		Hashtable<String, ContadorVerbo> conteo = new Hashtable<String, ContadorVerbo>();
		Debug.setPrefix("* ");
		Debug.setDebug( false );
		boolean sacarMucho = false;
		// Procesar verbos de us:
		...
		String fileOut4 = "d:\\t\\Verbos-conteados-201004.txt";
		String fileOut5 = "d:\\t\\Verbos-errores-201004.txt";
        File regFile = new File(fileName);
    	PrintStream logFile = null;
    	PrintStream logFile2 = null;
    	PrintStream logFile3 = null;
    	PrintStream logFile4 = null;
    	PrintStream logFile5 = null;
		try {
			if (sacarMucho) {
				logFile = new PrintStream( new FileOutputStream( fileOut, false /* append */ ) );
				logFile2 = new PrintStream( new FileOutputStream( fileOut2, false /* append */ ) );
			...
			logFile4 = new PrintStream( new FileOutputStream( fileOut4, false /* append */ ) );
			logFile5 = new PrintStream( new FileOutputStream( fileOut5, false /* append */ ) );
		} catch (FileNotFoundException e2) {
		}
    	if (regFile.exists()) {
    	    try {
		        ...
		        while ((line = input.readLine()) != null) {
		        	...
		        	if (!line.equals("")) {
		    			...
		    			String verbo = getVerboPrincipal( line );
		    			if (verbo.equals("")) {
		    				logFile5.println( line );
		    			} else {
			    			// logFile4.println( verbo );
			    			if (conteo.containsKey( verbo ))
			    				conteo.get( verbo ).inc();
			    			else
			    				conteo.put( verbo, new ContadorVerbo( verbo ) );
		    			}
		        	}
		        	numLin++;
		    		Debug.setDebug( true );
		...
		Collection<ContadorVerbo> lista = conteo.values();
		ArrayList<ContadorVerbo> listaVerbos = new ArrayList<ContadorVerbo>();
		for (ContadorVerbo cv : lista) {
			listaVerbos.add( cv );
		}
		Collections.sort( listaVerbos );
		System.out.println( listaVerbos );
		for (ContadorVerbo cv : listaVerbos) {
			logFile4.println( cv );
		}
		if (sacarMucho) {
	    	logFile.close();
	...
	/** Analiza un texto en busca de formas verbales
	 * @param texto	Texto a analizar
	 * @return	Lista de infinitivos de los verbos encontrados
	 */
	public static ArrayList<String> devolverInfinitivosVerbos( String texto ) {
		ArrayList<String> verbos = new ArrayList<String>();
		int numFrase = 0;
		int offsetInicio = 0;  // offset de inicio de la frase
		while (offsetInicio < texto.length()) {
			Expresion exp = analizaFrase( texto, offsetInicio, true, true );
			// 2. Añadimos a una expresión que solo tenga los verbos
			int posi = exp.buscaTexto( VerboConjugado.class, "", 0, false);
			while (posi >= 0) {
				verbos.add( ((VerboConjugado) exp.getTC(posi)).getInfinitivoVerboPrincipal() );
				posi = exp.buscaTexto( VerboConjugado.class, "", posi+1, false);
			}
			// 3. Avanzamos a la siguiente frase
			offsetInicio = exp.getOffsetFin();
	...
	/** Analiza un texto en busca de formas verbales
	 * @param texto	Texto a analizar
	 * @return	Lista de formas verbales de los verbos encontrados
	 */
	public static ArrayList<String> devolverFormasVerbales( String texto ) {
		ArrayList<String> verbos = new ArrayList<String>();
		int numFrase = 0;
		int offsetInicio = 0;  // offset de inicio de la frase
		while (offsetInicio < texto.length()) {
			...
			int posi = exp.buscaTexto( VerboConjugado.class, "", 0, false);
			while (posi >= 0) {
				verbos.add( exp.getTC(posi).getTextoUnidad() );
				posi = exp.buscaTexto( VerboConjugado.class, "", posi+1, false);
			}
			// 3. Avanzamos a la siguiente frase
			offsetInicio = exp.getOffsetFin();
	...
	public static void main( String[] s ) {
		// prueba1();
		// verbosTotales();
		String frase = "Aquí tengo un test de varios verbos que existen en una sola frase. A veces se empiezan a ver perífrasis. Y otro verbo simple que hay en esta";
		System.out.println( frase );
		ArrayList<String> verbos = devolverInfinitivosVerbos( frase );
		System.out.println( verbos );
		ArrayList<String> formasVerbales = devolverFormasVerbales( frase );
		System.out.println( formasVerbales );
	}
	
}


class ContadorVerbo implements Comparable<ContadorVerbo> {
	String verbo;
	int contador = 1;
	ContadorVerbo( String v ) {
		verbo = v;
	}
	public void inc() {
		contador++;
	...
	@Override
	public int compareTo(ContadorVerbo arg0) {
		if (contador < arg0.contador)
			return +1;
		else if (contador == arg0.contador)
			return 0;
		else return -1;
	}
	public String toString() {
		return verbo + "\t" + contador;
	}
}
